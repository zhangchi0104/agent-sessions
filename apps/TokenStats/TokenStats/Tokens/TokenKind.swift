...
//  TokenStats
//
//  The Token Odometer's third axis (CONTEXT.md): the three displayed ways a
...
/// One of the three Token Kinds, in the order the table's columns and the
...
nonisolated extension TokenUsage {
    ...
    /// instead would put these fields in a type that owns none of them.
    ...
    /// three-kind Token Odometer, while this figure drives only table subtotals,
    ...
    /// The objective token summary shared with Windows: direct input and
    /// output. Cache reads stay in the raw Odometer and table but are excluded
    /// from this specifically named presentation.
