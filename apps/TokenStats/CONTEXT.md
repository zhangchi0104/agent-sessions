...
**Selected total**:
A Windows display projection over the Token Odometer: the sum of the Token Kinds whose column headings are enabled, within the persisted Today, 7-day, or 30-day range. It drives the filtered total in the flyout and the Windows tray tooltip. Toggling a heading changes the selected total, not the parsed transcript records or the raw four-kind Token Odometer total. For a Model row, a Token Kind's displayed percentage is its share of that row's selected total; it is a composition percentage, never quota consumption. The Display preference chooses **value**, **percentage**, or **value (percentage)**.
